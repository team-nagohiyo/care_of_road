...
static PurchaseData _purchaseGetJsonItem(const string& key);
...
{
    return (int)([appCCloud showingAd] || [appCCloud showingCutin]);
...
vector<PurchaseData> AppCCloudPlugin::Purchase::getAllData()
{
    ...
    for (NSDictionary *item in items) {
        ...
        [ret_item setValue:[NSNumber numberWithInteger:current_count.integerValue] forKey:@"count"];
...
{
    PurchaseData data = _purchaseGetJsonItem(key);
    return data;
}
...
static PurchaseData _purchaseGetJsonItem(const string& key)
{
    vector<PurchaseData> allData = AppCCloudPlugin::Purchase::getAllData();
    vector<PurchaseData>::iterator it = allData.begin();
    for(;it!=allData.end();it++){
        if (key == it->getId()) {
            return *it;
        }
    }
